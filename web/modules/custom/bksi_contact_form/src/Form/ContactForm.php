...
class ContactForm extends FormBase
{
  ...
  public function buildForm(array $form, FormStateInterface $form_state)
  {
    ...
    // Add a submit button that handles the submission of the form if form  isnt already submited.
    if (empty($this->messenger()->all())) {
      $form['actions']['submit'] = [
        '#type' => 'submit',
        '#value' => $this->t('Submit'),
      ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $params = array(
      ...
      'attachments' => array_map(
        fn ($file) => ['filecontent' => file_get_contents($file), 'filename' => 'asdasd.pdf'],
        $files_url
      ),
    );
